  ...
  module HtmlProofer
    ...
    Jekyll::Hooks.register(:site, :post_write, priority: priority) do |site|
      if not site.config["proofer"] == false
        # lazy-load html-proofer only when proofing is enabled
        # (keeps local builds working without the CI-only dependency)
        begin
          require 'html-proofer'
        rescue LoadError
          warn "html-proofer not installed; skipping link proofing"
          next
        end
        options = {
